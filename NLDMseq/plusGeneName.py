...
import os
import os.path
def plusGeneName(refFlat_Check_GeneUnionExonLenFile,readFileList,targetDir,readType,annotation_type): 
		...
		for j in range(len(readFileList)):
			...
			while True:
			  ...
			  if not line:
				...
				if len(line)>=12:
					lineNum=lineNum+1;
					isfName1_long=line[2]

					if annotation_type =='refGene':
						isfName1_list=isfName1_long.split('_');
						isfName1=isfName1_list[2]+'_'+isfName1_list[3];
					if annotation_type =='ensGene':
						isfName1_list=isfName1_long.split('_');
						isfName1=isfName1_list[2]					
					if annotation_type=='Ensembl':
						isfName1=isfName1_long;	
					if annotation_type == 'knownGene':
						isfName1_list=isfName1_long.split('_');
						isfName1=isfName1_list[2]		 
					read1=line[0] 
					loc1=line[3]
		...
		for i in range(len(readFileList)):
			...
			for line in f_reads:
				...
				if len(line)>=12:
					...
					isfName_long=line[2]	
					if annotation_type =='refGene':
						isfName_list=isfName_long.split('_');
						isfName=isfName_list[2]+'_'+isfName_list[3];
					if annotation_type =='ensGene':
						isfName_list=isfName_long.split('_');
						isfName=isfName_list[2]					
					if annotation_type=='Ensembl':
						isfName=isfName_long;	
					if annotation_type == 'knownGene':
						isfName_list=isfName_long.split('_');
						isfName=isfName_list[2]		 
					read=line[0]
